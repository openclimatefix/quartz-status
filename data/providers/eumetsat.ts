...
type Details = {
  ...
  onTime: number;
  late: number;
  undeliveredPlanned: number;
  undeliveredUnplanned: number;
  total: number;
  results?: ResultDetails;
};
type ResultDetails = [
  [datetime: string, deliveryStatus: string, timelyStatus: string][],
  ["datetime", "deliveryStatus", "timelyStatus"]
];

const constructDateTime = (dateString: string): string => {
  ...
  try {
    ...
    const statusData = await page.evaluate(
      async ([verbose]) => {
        ...
        const resultDetails: ResultDetails = [[], ["datetime", "deliveryStatus", "timelyStatus"]];
        ...
        for (const row of rows) {
          ...
          const timestamp = cells[1].textContent?.trim() || "Unknown";

          // Set the latest timestamp if it's empty
          if (details.latestTimestamp === "") details.latestTimestamp = timestamp;
          ...
          if (verbose) {
            // @ts-ignore
            const datestamp = await window.constructDateTime(`${date} ${timestamp}`);
            resultDetails[0].push([datestamp, deliveryStatus, timelyStatus]);
          }
        }
